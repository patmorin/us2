...
\Cref{d_degenerate_upper_bound} follows easily from \cref{degenerate_and_degree}, by the following argument.  Since $G$ is $d$-degenerate, it has at most $dn$ edges and the sum of vertex degrees is at most $2dn$.  Therefore, the set $S:=\{v\in V(G):\deg_G(v)\ge n^{2/3}/\log n\}$ has size at most $2dn/n^{2/3}=2dn^{1/3}\log n$.  Since $G-S$ has maximum degree $\Delta\le n^{2/3}/\log n$, $G-S$ has a us-colouring $\varphi$ using at most $cd\Delta\log^{3/2} n = cdn^{1/3}\log  n$ colours, by \cref{degenerate_and_degree}. We can extend $\varphi$ to a colouring of $G$ by assigning each vertex in $S$ a distinct colour that is larger than any colour used in the colouring of $G-S$.  Thus, $\trn(G)\le cdn^{1/3}\log  n+2dn^{1/3}\le c'dn^{1/3}\log  n$ for some $c'>2c$.
...
\begin{proof}[Proof of \cref{degenerate_and_degree}]
  ...
  Therefore, $|\outn(v)|$ is dominated by a sum $X:=\sum_{w> v} N'_{v,w}$ of independent random variables.  In the appendix, we show how to use one of Bernstein's Inequalities to prove that $\Pr\left(X\ge Cd\sqrt{\Delta}\log^
  {3/2} n\right)\le n^{-\Omega(C/d)} = o(1/n)$ when $C$ is a sufficiently large multiple of $d$.  This completes the proof.
\end{proof}
...
\[
  ...
  = \frac{x_i}{k}(1+H_{k}-H_{x_i/k}) = \frac{x_i\ln x_i}{k}\left(1 \pm o(1) \right)
  = O\left(\frac{x_i\log x_i}{k}\right)
\]
so
\[
  (\E(X_i))^2 = O\left(\left(\frac{x_i\ln x_i}{k}\right)^2\right) = O(x_i\log^2 x_i) \enspace ,
\]
since $x_i\le k^2$,
and
\begin{align*}
  ...
  & \le \frac{x_i}{k^2}\left(k^2 + (\E(X_i))^2\right)
    + \frac{1}{k}\cdot \sum_{j=x_i/k+1}^k \left(\frac{x_i}{j}-\E(X_i)\right)^2 \\
  & = x_i + O\left(\frac{x_i^2\log^2 x_i}{k^2}\right)
      + \frac{1}{k}\cdot\sum_{j=x_i/k+1}^k \left(\frac{x_i}{j}-\E(X_i)\right)^2 \\
  & \le O(x_i\log^2 x_i)
      + \frac{1}{k}\cdot\sum_{j=x_i/k+1}^k \left(\frac{x_i}{j}-\E(X_i)\right)^2
      & \text{(since $x_i\le k^2$)} \\
 & = O(x_i\log^2 x_i) + \frac{1}{k}\sum_{j=x_i/k+1}^k \left(\frac{x_i^2}{j^2}-\frac{2x_i \E(X_i)}{j} + (\E(X_i))^2\right) \\
 & \le O(x_i\log^2 x_i) + \frac{1}{k}\sum_{j=x_i/k+1}^k \left(\frac{x_i^2}{j^2} + (\E(X_i))^2\right) \\
 & = O(x_i\log^2 x_i) +  \left(\frac{x_i^2}{k}\sum_{j=x_i/k+1}^k\frac{1}{j^2}\right)
   + \left(\sum_{j=x_i/k}^k O\left(\frac{x_i\log^2 x_i}{k}\right)\right) \\
 & \le O(x_i\log^2 x_i) + \left(\frac{x_i^2}{k}\sum_{j=x_i/k+1}^k\frac{1}{j^2}\right)
    + O(x_i\log^2 x_i)\\
  & \le O(x_i\log^2 x_i) + \frac{\pi^2 x_i}{6} + O(x_i\log^2 x_i) \\
  & = O(x_i\log^2 x_i)
\end{align*}
...
\begin{align*}
  ...
  & \le \exp \left(-\frac{\tfrac{1}{2}\sqrt{\Delta}\log^{3/2} n)^2}{O(d\Delta\log^2\Delta) + \tfrac{1}{3}kt}\right) \\
  & \le \exp \left(-\frac{\tfrac{1}{2}C^2\Delta\log^3 n}{O(d\Delta\log^2\Delta) + \tfrac{1}{3}kt}\right) \\
  & \le \exp \left(-\frac{\tfrac{1}{2}(C\Delta\log n)^2}{O(d\Delta\log^2\Delta) + \tfrac{1}{3}C\Delta\log^{3/2}\Delta}\right) & \text{(since $k=\sqrt{\Delta}$)} \\
  & \le \exp \left(-\frac{\tfrac{1}{2}(C\Delta\log n)^2}{O(d\Delta\log^2n) + \tfrac{1}{3}C\Delta\log^{3/2}n}\right)
  & \text{(since $\Delta \le n$)} \\
  & \le \exp \left(-\Omega((C/d)\log n)\right) \\
  & \le n^{-\Omega(C/d)} \enspace .
\end{align*}

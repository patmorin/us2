...
\begin{thm}
  ...
  for all integers $d\ge 1$, $\Delta \ge 1$, and $n> \Delta$, every $d$-degenerate graph of maximum degree $\Delta$ has a us-colouring using at most $C d^?\sqrt{\Delta}\log n$ colours.
\end{thm}

\begin{proof}
  Let $S_0:=V(G)$ and, for each integer $i\ge 1$, let $S_i:=\{v\in S_{i-1}:\deg_{G[S_{i-1}]}(v)\ge 4d\}$.  Since $G$ is $d$-degenerate $G[S_{i-1}]$ has at most $d|S_{i-1}|$ edges and total degree at most $2d|S_{i-1}|$, so $|S_i|\le |S_{i-1}|/2$ for each $i\ge 1$.  Let $k$ be the maximum integer such that $S_i$ is non-empty.  For each $i\in\{0,\ldots,k\}$, let $L_i:=S_i\setminus S_{i+1}$.

  We will construct a us-colouring $\varphi:V(G)\to \N$ using a product colouring.  For each $i\in\{0,\ldots,k\}$, we compute a proper colouring of the graph $(G[L_i])^2$.  Since each vertex in $L_i$ has degree less than $4d$ in $G[S_i]\subseteq G[L_i]$, this colouring requires at most $16d^2$ colours, and is one factor in the product colouring of $G$.
  The second factor of the product colouring will be computed using a two-phase process. In the first phase we use a sequence of color palettes $\Phi_0,\ldots,\Phi_{k}$, where $\Phi_i:=\{2i\sqrt{\Delta}+1,\ldots,2(i+1)\sqrt{\Delta}\}$ and is used to colour the vertices of $L_i$, for each $i\in\{0,\ldots,k\}$.

  Fix an arbitrary total order $<$ on the vertices of $S_i$ for each $i\in\{0,\ldots,k\}$ and define the total order $<$ on $V(G)$ in which $v <w$ if $v\in L_i$, $w\in L_j$, and $i<j$ or if $v,w\in L_i$ and $v<w$.  Let $H$ be the directed acyclic graph obtained from $G^2$ by directing each edge $vw$ as $\overrightarrow{vw}$ so that $v<w$.  We claim that each vertex $v$ of $H$ has out-degree at most $(4d-1)(2\Delta+1)$ in $H$.  To see this, consider some edge $\overrightarrow{vw}$ originating at $v$ and suppose $v\in L_i$.  The existence of $\overrightarrow{vw}$ in $H$ implies at least one of the following:
  \begin{compactenum}
    \item $vw\in E(G)$.  Since $v<w$, $w\in S_i$.  Since $v\in L_i$, $\deg_{G[S_i]}(v)\le 4d-1$, so these types of edges contribute at most $4d-1$ to the out-degree of $v$.
    ...
    \item $G$ contains a path $vyw$ with $v < w < y$.  In this case, there are at most $4d-1$ choices for $y$ and, for each such $y$ there are at most $\Delta$ choices for $w$.  Therefore, tehse types of vertices contribute at most $(4d-1)\Delta$ to the out-degree of $v$.
  \end{compactenum}
  We consider the vertices in the order given by $<$.  When colouring some vertex $w\in L_i$ we count, for each colour $\alpha\in \Phi$, the number $N_\alpha(w)$ of neighbours $y\in N_G(w)$ such that $y < w$ and $y$ already has a neighbour $v$ of colour $\alpha$.  We choose the colour of $w$ uniformly at random from the set $\Phi_w:=\Phi_i\setminus \{\alpha\in\Phi_i: N_{\alpha}(w)\ge\sqrt{\Delta}\}$.  Since $\sum_{\alpha\in\Phi_i} N_\alpha(w)=\deg_G(w)\le\Delta$, the set $\Phi_w$ has size at least $2\sqrt{\Delta}-\sqrt{\Delta}=\sqrt{\Delta}$.
  This completes the description of the first-phase colouring $\varphi$ of $G$.

  At this point, the only violations of the us-colouring conditions occur at three-vertex paths $vyw$ with $v,w\in L_j$ for some $j\in\{0,\ldots,k\}$ and $w\in L_i$ for some $j\in\{0,\ldots,j-1\}$.  We say that a vertex $y$ is \defin{problematic} if $y$ has neighbours $v$ and $w$ in $G$ such $y\in L_i$, $v,w\in L_j$, $i < j$ and $v$ and $w$ receive the same colour in phase one.

  At this point, we can justify the choice of the colour set $\Phi_w$ that we choose from when colouring $w$. For each promatic vertex $y$, there exists a minimum vertex $w$ (with respect to $<$) such that $y$ becomes problematic precisely when $w$ is coloured.  When this happens, we say that $w$ \defin{completes} $y$. By definition, if $w$ chooses the colour $\alpha\in\Phi_w$, then it completes at most $N_{\alpha}<\sqrt{\Delta}$ vertices.

  Let $P$ be the set of problematic vertices.  We will re-colour every vertex in $P$ with a colour in the palette $\Phi_{k+1}:=\{(k+1)\sqrt{\Delta}+1,\ldots,(k+1)\sqrt{\Delta}+Cd\sqrt{\Delta}\log n+1\}$.  Since the colours in $\Phi_{k+1}$ are larger than any colours used thus far, the only further violations of the us-colouring conditions that could occur would be caused by paths (with two or three vertices) whose endpoints are in $P$.  In order to avoid these we will properly colour the induced subgraph $G^2[P]$.  To show that this is possible, we will prove that, with high probability, the maximum out-degree of each vertex in $H[P]$ is at most $Cd\sqrt{\Delta}\log n$.  Since $H$ is acyclic, this implies that $G^2[P]$ is $Cd\sqrt{\Delta}\log n$-degenerate, so the $Cd\sqrt{\Delta}\log n$ colours in $\Phi_{k+1}$ are sufficient to properly colour it.

  We now focus on a specific vertex $v\in V(G)$ and show that, with probability $1-o(1/n)$, the number of edges $\overrightarrow{vy}$ in $H$ such that $y\in P$ is at most $Cd\sqrt{\Delta}\log n$.  This show, in particular, that with probability $1-o(1)$, that each $v\in P$ has out-degree at most $Cd\sqrt{\Delta}\log n$ in $H[P]$, as required.

  Let $N_H(v):=\{y\in V(H): \overrightarrow{vy}\in E(H)\}$. If some vertex $w$ completes $y\in N_H(v) $, then we say that $w$ \defin{$v$}-completes $y$.  At the time $w\in L_j$ is coloured, at most $d-1$ neighbours of $y$ in $L_j$ have already been coloured.  Therefore, for a fixed $y\in N_H(v)$ and fixed $w\in N_G(y)$, the probability that $w$ $v$-completes $y$ is at most $(d-1)/\sqrt{\Delta}$.

  For any pair of vertices $y,w\in V(H)$, define the indicator random variable
  \[
    I_{y,w} := \begin{cases}

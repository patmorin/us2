...
\begin{proof}
  Let $<$ be a total order on the vertices of $G$ such that each vertex $v$ of $G$ has at most $d$ neighbours $u$ such that $u<v$.  For each edge $vw$ of $G^{\ell}$, select some path $\Pi_{vw}:=x_0,\ldots,x_r$ in $G$ with endpoints $x_0:=v$ and $x_r:=w$ and having $r\le\ell$ edges.  Say that an edge $x_ix_{i+1}$ of $\Pi{vw}$ is a \defin{downstream} edge if $x_i < x_{i+1}$, otherwise it is an \defin{upstream} edge.  If at least half the edges of $\Pi_{vw}$ are downstream edges then orient the edge $vw$ as $\overrightarrow{vw}$, otherwise orient it as $\overleftarrow{vw}$.  We claim that the resulting oriented version of $G^{\ell}$ has maximum out-degree at most $2^{\ell+1} d^{\lceil\ell/2\rceil}\Delta^{\lfloor\ell/2\rfloor}$.  To see this, observe that for each edge $\overrightarrow{vw}$ we can describe the path $\Pi_{vw}$ as follows.  We first specify length $r$ of the path, then a sequence of $r$ bits that describe which edges of $\Pi_{vw}$ are upstream and which are downstream.  Given $x_i$, if $x_ix_{i+1}$ is a downstream edge then there are at most $d$ options for $x_{i+1}$.  If $x_ix_{i+1}$ is an upstream edge then there are at most $\Delta$ options for $x_{i+1}$.  For a given $r$, the number of such paths is therefore at most $2^rd^{\lceil r/2\rceil}\Delta^{\lfloor r/2\rfloor}$, since at least half the edges of $\Pi_{vw}$ are downstream edges.  Summing over $r$ from $1$ to $\ell$ establishes the claim.  Therefore $G^{\ell}$ has an orientation in which each vertex has maximum out-degree $2^{\ell+1} d^{\lceil\ell/2\rceil}\Delta^{\lfloor\ell/2\rfloor}$.  This implies that $G^{\ell}$ is $2^{\ell+2} d^{\lceil\ell/2\rceil}\Delta^{\lfloor\ell/2\rfloor}$-degenerate, as required.
\end{proof}
...
\begin{proof}
  Let $\Delta:=n^{2/(\ell+2)}$ and let $S:=\{v\in V(G):\deg_G(v)\ge\Delta\}$.  Then $G-S$ is $d$-degenerate and has maximum-degree $\Delta$, so $(G-S)^{\ell}$ is $O(\Delta^{\ell/2})$-degenerate and therefore has a proper colouring using $O(\Delta^{\ell/2})=O(n^{\ell/(\ell+2)})$ colours.  This extends to an $\ell$-vertex-ranking of $G$ by giving each vertex in $S$ a distinct colour large than all vertices of $G-S$.  This requires an additional $n/\Delta=n^{\ell/(\ell+2)}$ colours.
\end{proof}
...
\begin{thm}\label{l_degenerate_and_degree}
  For any integers $d\ge 2$ and $\ell\ge 2$ there exists a constant $c:=c(d,\ell)$ such that, for all integers $\Delta\ge d$ and $n\ge \Delta$, every $n$-vertex $d$-degenerate graph $G$ of maximum degree $\Delta$ has $\lrn(G)\le c\Delta^{\ell-3/2}\log n$.
\end{thm}

% \todo[inline]{If we want to improve the overall result, we should make use of the fact that $\Delta$ is quite big. Below, we set $\Delta=n^{1/(\ell-1/2)}$,
% so the bound $|N^+_H(p)|\le\Delta^{\ell-1}=n^{1-1/(2\ell-1)}$ is already very close to $n$, and the bound $\sum_{w}x_w \le \Delta^{2\ell-3}=n^{(2\ell-3)/(\ell-1/2)}=n^{2\ell\pm o(1)}$ is much bigger than $n$.  I don't think this is a achievable in a graph with constant degeneracy.
% }
%
% \todo[inline]{For example, by Cairn's argument, the number of paths of length $\ell$ with both endpoints in $L_j$ is at most $2|L_j|d^{\lceil\ell/2\rceil}\Delta^{\lfloor\ell/2\rfloor}=O(|L_j|\Delta^{\ell/2})$.  Each such path contributes at most $\ell$ to $\sum_w x_w$, so $\sum_w x_w\le \sum_{i}|L_i|\Delta^{\ell/2}= O(n\Delta^{\ell/2})=n^{1+\ell/4\pm o(1)}$.
% }

% \todo[inline]{The more I think about, the more it looks like the answer could be something like $O(n^{\ell+2}{\ell+4})$}

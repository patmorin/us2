...
The current work is motivated by the gap between the upper and lower bounds for $\rn{2}(G)$ for $d$-degenerate graphs $G$. For fixed $d$, the upper bound is $O(\sqrt{n})$, while the lower bound is $\Omega(n^{1/3})$ and closing this gap is stated explicitly as an open problem by \citet{karpas.neiman.ea:on} and \citet{bose.dujmovic.ea:asymptotically}. Our first contribution solves
this problem, up to a logarithmic factor.

\begin{thm}\label{unique_superior}
  For any positive integers $d$ there exists a constant $c:=c(d)$ such that, for every integer $n\ge d$, every $n$-vertex $d$-degenerate graph $G$ has $\rn{2}(G)\le cn^{1/3}\log n$.
\end{thm}
...
\begin{thm}\label{l_degenerate_and_degree}
  For all positive integers $d$ and $\ell\ge 2$ there exists a constant $c:=c(d,\ell)$ such that, for every integer $\Delta\ge d$, every integer $n\ge \Delta$, every $n$-vertex $d$-degenerate graph $G$ of maximum-degree at most $\Delta$ has
  \[
...
\begin{lem}\label{advanced_cairns2}
  ...
  \begin{compactenum}[(i)]
    \item $\rho(\Pi)\in V(\Pi)$ for each $\Pi\in\mathcal{P}$; and
    \item $|\rho^{-1}(v)| \le 2^{\ell}d^{\lceil \ell/2\rceil+1}\Delta^{\lfloor\ell/2\rfloor-1}$ for each $v\in V(G')$.
  \end{compactenum}
\end{lem}

\begin{proof}
  The proof is almost identical to \cref{advanced_cairns2} with two modifications.  If $G'$ contains both edges $\overrightarrow{vw}$ and $\overrightarrow{wv}$ then this edge is considered as a downstream edge no matter which direction it is traversed, since there are at most $d$ options for edges leaving $v$ (one of which is $w$) and at most $d$ options for edges leaving $w$ (one of which is $v$).  The second change is that, by setting, $\rho(x_0,\ldots,x_r)$ to one of $x_1$ or $x_{r-1}$ the number of options for describing $x_0,\ldots,x_r$ decreases.  Specifically, for $r\ge 3$, $\Pi:=x_0,\ldots,x_r$ can be described by a path of length $1$ with one downstream edge (for which there are $d$ options) and a path of length $r-1$ with at most $\lfloor (r-2)/2\rfloor=\lfloor r/2\rfloor-1$ upstream edges (for which there are $2^{r-1}d^{\lceil r/2\rceil+1}\Delta^{\lfloor r/2\rfloor-1}$ options).  Thus, the number of paths assigned to any vertex is at most $2^{r-1}d^{\lceil r/2\rceil+1}\Delta^{\lfloor r/2\rfloor-1}$ for $r\ge 3$ (and at most $d^r$ for $r\in\{1,2\}$).
\end{proof}
...
\begin{proof}[Proof of \cref{l_degenerate_and_degree}]
  ...
  Consider a problematic path $\Pi:=x_0,\ldots,x_r$ and suppose, without loss of generality that $\tau(\Pi)=x_0$.  Then $x_r$ was coloured before $x_0$ and $\Pi$ became problematic precisely when $x_0$ was coloured.  By the time the colouring is complete, a problematic path $\Pi$ may be an $\ell$-violation.  In order to eliminate this potential $\ell$-violation we declare the vertex $\gamma(\Pi)$ to be \defin{problematic}.
  ...
  We want to show that $G^\ell[P]$ has chromatic number at most $ck\log n+1$.  To do this, we will show that the maximum out-degree of $H[P]$ is at most $ck\log n$, with high probability.  In fact, we will show something stronger: that every vertex $p$ in $H$ has at most $ck\log n$ out-neighbours in $P$.
...
This gives a lower bound of $\Omega(n^{1/2})$ for $\ell=4$ and our upper bound in this case is $O(n^{3/5}\log n)$.  I don't see how to get any further with constructions like this. The problem is that in order to prove a lower bound of $k$, this construction requires a set $A$ of $k$ vertices such that, for any two vertices in $A$, there are $k$ interior-disjoint paths between them.  This requires that each vertex in $A$ has degree at least $k$, for a total degree of $k^2$.  But if the graph is $d$-degenerate, then it $dn \ge k^2$, so $k\le \sqrt{dn}$.  In other words, we can never prove a bound greater than $\sqrt{dn}$ this way, no matter how big we make $\ell$.  Another way to think of it is that any construction we make should not have vertices of degree greater than $2dn/k$, since there are at most $k$ such vertices so we could remove them beforehand and give them all distinct large colours later.  If we're shooting for the optimal bound $k=n^{3/5}$ for $\ell=4$ or $\ell=5$ then we shouldn't have any vertices of degree greater than $n^{2/5}=k^{2/3}$.

This leads me to the following conjecture: Make a graph $G$ from a bipartite graph with parts $A$ and $C$ of sizes $k$ and $k^{5/3}$.  Each vertex in $C$ chooses $2$ random neighbours in $A$.  Then we subdivide each edge so that $G$ is $2$-degenerate.  I conjecture that $\lrn(G)\ge \Omega(k^{1-\epsilon})$ for any $\epsilon >0$.

% Let's try to do better.  Make a bipartite graph with parts $A$ and $C$ of sizes $k$ and $k^{9/5}$, respectively.  Each vertex in $C$ picks $2$ random neighbours in $A$.  Then, for a vertex $v\in A$, $\E(N_G(v))\approx k^{4/5}$.  Going one step further the expected number of vertices in $A$ at distance $2$ from $v$ is roughly $k^{4/5}$ since each vertex in $N_G(v)$ has a second edge leading to a random vertex in $A$.

% Now fix distinct $v,w\in A$ arbitrarily.  The expected number of common neighbours of $v$ and $w$ is $k^{9/5}/\binom{k}{k} = \Theta(k^{-1/5})$ and, with high probability, $v$ and $w$ have at most $c\log k$ common neighbours.  If we eliminate these, then each of $v$ and $w$ still has roughly $k^{4/5}$ random vertices in $A$ at distance $2$.  If we take two $k^{4/5}$-element random subsets of a $k$-element set, the expected number of things that appear in both subsets is approximately $




% For $\ell=8$, the following construction works:  Make a graph with $3$ layers $A$, $B$, and $C$, each of size $k$.  Each vertex in layer $A$ chooses $\sqrt{k}$ random neighbours in layer $B$. Each vertex in layer $B$ chooses $\sqrt{k}$ random neighbours in layer $C$.  Now subdivide every edge of this graph so that it is $2$-degenerate.  This graph has $n:=O(k^{3/2}$ vertices. Note that, for each vertex in $A$, there are $\Theta(n)$ ``random'' vertices in $C$ that can be reached from $A$ by a path of length $4$.  It should be easy to argue that, for any pair of distinct vertices $v,w\in A$, there are $\Theta(k)$ disjoint paths from $v$ to $w$, each of length $8$. Again, if we colour this graph with less than $k$ colours then two vertices in $A$ get the same colour and the we must use $\Omega(k)=\Omega(n^{2/3})$ larger colours on theese paths.

% I guess this generalizes to $\ell=4a$ for any fixed integer $a$ and gives a lower bound of
% \[ \Omega(n^{a/(a+1)}) = \Omega(n^{1-\frac{4}{\ell+4}}) \enspace . \]
% The construction is similar, but you start with a graph with $a+1$ layers $A_0,\ldots,A_a$ each of size $k$ and each vertex in $A_i$ chooses $k^{1/a}$ random neighbours in $A_{i+1}$.  This gives a graph with $\Theta(k^{1+1/a})$ edges, which you subdivide to get a graph with $n:=\Theta(k^{1+1/a})$ vertices.  An argument like the one above shows that this graph needs $\Omega(k)=\Omega(n^{1-4/(\ell+4)})$ colours.

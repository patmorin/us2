...
As a warm-up, consider the graph $G$ obtained from the Erd\H{o}s-Renyi random graph $G_{n,\Delta/n}$ by subdividing each edge once.  This graph is $2$-degenerate and has maximum degree $\Delta\log^{O(1)} n$.  I conjecture that, with high probability, $\rn{4}(G)\ge \Delta^{3/2}/\log^{O(1)} n$.  More generally, for any fixed even integer $\ell$, $\lrn(G)\ge \Delta^{(\ell-1)/2}/\log^{O(1)} n$.

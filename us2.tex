...
\newcommand{\jedrzej}[1]{\textcolor{ProcessBlue}{Jędrzej: #1}}
\newcommand{\piotr}[1]{\textcolor{orange}{Piotr: #1}}
...
which plays a central role in the theory of sparsity  \cite{nesetril.ossona:tree-depth,nesetril.ossona:sparsity}.
At the other extreme, when $\ell=2$, a $2$-vertex-ranking of $G$ is also known as a \defin{restricted star colouring} \cite{shalu.antony:complexity} or a \defin{unique superior colouring} of $G$,
see \cite{karpas.neiman.ea:on}.  Previous results for $\ell=2$ and fixed $\ell\ge 2$ are summarized in \cref{previous_works}.

\begin{table}
    \centering{
        \begin{tabular}{|l|l|l|l|} \hline
          \multicolumn{4}{|c|}{$\rn{2}$} \\\hline
            Graph class & Lower Bound & Upper Bound & Ref. \\ \hline
            Trees & $\Omega(\log n/\log\log n)$ & $O(\log n/\log\log n)$ & \cite{karpas.neiman.ea:on} \\
            Planar graphs & $O(\log n/\log^{(3)} n)$ & $O(\log n/\log^{(3)} n)$ & \cite{bose.dujmovic.ea:asymptotically} \\
            Proper minor closed & $\Omega(\log n/\log\log n)$ & $O(\log n)$ & \cite{karpas.neiman.ea:on} \\
            $d$-cubes & $d+1$ & $d+1$ & \cite{almeter.demircan.ea:graph} \\
            Max-degree 3 & $6$ & $7$ & \cite{almeter.demircan.ea:graph} \\
            Max-degree $\Delta$ & $\Omega(\Delta^2/\log \Delta)$ & $O(\min\{\Delta^2,\Delta\sqrt{n}\})$ & \cite{karpas.neiman.ea:on,almeter.demircan.ea:graph} \\
            $d$-degenerate & $\Omega(n^{1/3} + d^2/\log d)$ & $O(d\sqrt{n})$ & \cite{karpas.neiman.ea:on,almeter.demircan.ea:graph} \\
            \hline \multicolumn{4}{c}{} \\
            \hline
            \multicolumn{4}{|c|}{$\rn{\ell}$, for fixed $\ell\ge 2$} \\\hline
            Simple treewidth $\le t$ & $\Omega(\log n/\log^{(t)} n)$ & $O(\log n/\log^{(t)} n)$ & \cite{bose.dujmovic.ea:asymptotically} \\
            Treewidth $\le t$ & $\Omega(\log n/\log^{(t+1)} n)$ & $O(\log n/\log^{(t+1)} n)$ & \cite{bose.dujmovic.ea:asymptotically} \\
            Planar graphs & $\Omega(\log n/\log^{(3)} n)$ & $O(\log n/\log^{(3)} n)$ & \cite{bose.dujmovic.ea:asymptotically} \\
            Outerplanar graphs & $\Omega(\log n/\log^{(2)} n)$ & $O(\log n/\log^{(2)} n)$ & \cite{bose.dujmovic.ea:asymptotically,karpas.neiman.ea:on} \\
            Genus-$g$ graphs & $\Omega(\log n/\log^{(3)} n)$ & $O(g\log n/\log^{(3)} n)$ & \cite{bose.dujmovic.ea:asymptotically} \\
...
The current work is motivated by the gap between the upper and lower bounds for $\rn{2}(G)$ for $d$-degenerate graphs $G$.  A graph $G$ is \defin{$d$-degenerate} if its vertices have an ordering $v_1,\ldots,v_n$ such that the degree in $v_i$ in the induced subgraph $G[\{v_1,\ldots,v_i\}]$ is at most $d$, for each $i\in\{1,\ldots,n\}$. For fixed $d\ge 2$, the upper bound is $O(\sqrt{n})$, while the lower bound is $\Omega(n^{1/3})$. Closing this gap is stated explicitly as an open problem by \citet{karpas.neiman.ea:on} and \citet{bose.dujmovic.ea:asymptotically}. Our first contribution solves
...
\begin{obs}\label{alternate_def}
  For any $\ell\ge 2$, a vertex colouring $\varphi:V(G)\to\{1,\ldots,k\}$ of a graph $G$ is an $\ell$-vertex ranking of $G$ if and only if, for each path $v_0,\ldots,v_r$ in $G$ with at most $\ell$ edges, $\varphi(v_0)\neq \varphi(v_r)$ or $\max\{\varphi(v_1),\ldots,\varphi(v_{r-1})\}>\varphi(v_0)$.
\end{obs}

\begin{proof}
  First, suppose $\varphi$ is an $\ell$-vertex ranking of $G$.  Let $v_0,\ldots,v_r$ be an arbitrary path in $G$ of length $1\le r\le \ell$.  We must show that $\varphi(v_0)\neq \varphi(v_r)$ or that $\varphi(v_0)<\max\{\varphi(v_1),\ldots,\varphi_{r-1}\}$.  Since $P=v_0,\ldots,v_r$ is a connected subgraph of $G$ whose diameter is $r\le\ell$,  there is exactly one $i\in\{0,\ldots,r\}$ such that $\varphi(v_0)=\max\{\varphi(v_0),\ldots,\varphi(v_r)\}$. If $i\in\{0,r\}$ then $\varphi(v_0)\neq\varphi(v_r)$, otherwise $\varphi(v_0)<\varphi(v_i)=\max\{\varphi(v_1),\ldots,\varphi(v_{r-1})$.

  Next, suppose $\varphi$ is a colouring of $G$ with the property that, for every path $v_0,\ldots,v_r$ in $G$ of length $r\le\ell$, $\varphi(v_0)\neq\varphi(v_r)$ or $\varphi(v_0)<\max\{\varphi(v_1),\ldots,\varphi(v_{r-1})$.  Let $H$ be an arbitrary connected subgraph of $G$ of diameter at most $\ell$.  We must show that there is exactly one $v\in V(H)$ such that $\varphi(v)=\max\{\varphi(w):w\in V(H)\}$.  Suppose, for the sake of contradiction, that there are actually two such vertex $v$ and $w$.  Let $v_0,\ldots,v_r$ be a shortest path, in $H$, with endpoints $v=v_0$ and $w=v_r$.  Since $H$ has diameter at most $\ell$, $r\le \ell$.  But then $\varphi(v_0)=\varphi(v_r)\ge\max\{\varphi(v_1),\ldots,\varphi(v_{r-1})\}$, which contradicts the definition of $\varphi$.
\end{proof}
...
An \defin{orientation} of an undirected (multi)graph $G$ is a directed graph that can be obtained from $G$ by choosing a direction for each edge of $G$.

We repeatedly make use of the following foklore result:

\begin{obs}\label{orientation_to_degeneracy}
  If an undirected multigraph $G$ has an orientation in which each vertex has out-degree at most $d$, then $G$ is $2d$-degenerate.
\end{obs}
...
A \emph{path} is a graph 
with the vertex set $\set{v_0,\ldots,v_k}$ and the 
edges $\set{v_{i-1}v_{i} : i \in [k]}$, where $v_0,\dots,v_k$ are pairwise distinct. 
%The number of vertices of a path is its \emph{order}.
We often refer to a path by a natural sequence of its vertices, 
writing, say, $v_0\cdots v_k$ or equivalently $v_k \cdots v_0$.
 %\michal{Is there an ambiguity with the edge notation \(uv\)?}
 %\jedrzej{It's the same in Diestel, we think that we should just leave it as it is.}
 % \michal{I am fine with that, although we may consider making a small remark. I leave it up to your taste}
Writing a path as $v_0\cdots v_k$ fixes an underlying orientation of the path, where $v_0$ is the first vertex and $v_k$ is the last vertex.
We say that $v_0$ and $v_k$ are the \emph{endpoints} of $v_0 \cdots v_k$.
We also say that $v_0 \cdots v_k$ \emph{starts} in $v_0$ and \emph{ends} in $v_k$.
%When $W=v_0\cdots v_k$ is a path and $i,j \in \{0,\dots,k\}$ we write $v_i[W]v_j$ for the \emph{subpath} $v_i\cdots v_j$.
%A \emph{cycle} is a graph with at least three vertices such that removing each of its edges gives a path. 
% \jedrzej{We changed the notation for paths. Now, we use "between" for paths and "from to" for witnessing path when we know the ordering. Keep this in mind and try to change succesively.}
%A \emph{forest} is a graph with no cycles.
%A \emph{tree} is a connected forest.
%Let $T$ be a tree, and let $u,v$ be two vertices in $T$.
%We write $u[T]v$ to denote the unique path in $T$ with endpoints $u$ and $v$ and with an orientation from $u$ to $v$. 
%We consider $u[T]v$ to have a fixed underlying orientation from $u$ to $v$.


\piotr{End of new text.}

In most works, including the current paper, a \defin{path} in a graph $G$ is defined as a sequence of distinct vertices $v_0,\ldots,v_r$ such that $v_{i-1}v_i\in E(G)$ for each $i\in\{1,\ldots,r\}$. A path $v_0,\ldots,v_r$ is often treated as a subgraph $P$ of $G$ with vertex set $V(P):=\{v_0,\ldots,v_r\}$ and $E(P):=\{v_{i-1}v_i:i\in\{1,\ldots,r\}\}$.  In the following, we count subgraphs of $G$ (like $P$) that correspond to paths. To distinguish between these two types of objects (sequences of vertices versus subgraphs of $G$) we introduce the following definition.

...
\begin{proof}[Proof Sketch]
  ...
  Let $\mathcal{P}$ contain all the undirected paths $x_0,\ldots,x_r$ in $\mathcal{P}_{\ell}(G)$ such that $x_0$ and $x_r$ are in the same layer $L_j$ and $\{x_1,\ldots,x_{r-1}\}\subseteq \bigcup_{i=1}^j L_i$.  Let $\rho:\mathcal{P}\to V(G)$ be the mapping given by \cref{advanced_cairns}, restricted to the paths in $\mathcal{P}$.  Let $\Pi:=x_0,\ldots,x_r$ be a path in $\mathcal{P}$ such that $\rho(\Pi)=x_0$.  We say that $\Pi$ is \defin{problematic} if $x_0$ and $x_r$ are assigned the same colour in the first phase of colouring.  When this happens, we say that the vertex $x_1$ is \defin{problematic}.

  We now describe the first phase colouring algorithm.  Consider the multigraph $G^*$ that, for each $vw\in V(G)$ contains as many copies of the edge $vw$ as there are undirected paths in $\mathcal{P}$ with endpoints $v$ and $w$.  The existence of $\rho$ implies that $G^*$ has an orientation in which each edge has out-degree $O(\Delta^{\lfloor\ell /2})$.  This implies that any induced subgraph $Q$ of $G^*$ has $O(|V(Q)|\Delta^{\lfloor\ell /2})$ edges and therefore contains a vertex of degree $O(\Delta^{\lfloor\ell /2})$.  Thus $G^*$ is $O(\Delta^{\lfloor\ell /2})$-degenerate.  Order the vertices of $G$ in the order $v_1,\ldots,v_n$ so that $v_i$ has degree $O(\Delta^{\lfloor\ell /2})$ in $G^*[\{v_1,\ldots,v_{i}]$.
  ...
  Let $P$ be the set of problematic vertices after phase one.  Let $H$ be the directed multigraph with vertex set $V(G)$ that, for each vertex $p$ of $G$ and each path $p\rightsquigarrow y$ in $\tau^{-1}(p)$ contains a copy of the edge $\overrightarrow{py}$.  Then the maximum out-degree of any vertex in $H$ is $O(\Delta^{\lfloor\ell /2\rfloor})$

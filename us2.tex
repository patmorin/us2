...
\begin{proof}[Proof of \cref{degenerate_and_degree}]
  ...
  \todo[inline]{There may actually be a problem here.  The value $M_w$ depends on $N^-_G(w)$.  It doesn't (and can't) take into account $|N^-_G(w)\cap P|$.  We need an upper bound on $\sum_{w\in C_p}M_w$.  Right now, the only upper bound I can see is $\sum_{w\in C_p} 4d|N^-_G(w)|/k \le |C_p|\cdot 4d\Delta/k= O(d^3\Delta^2/k)$.  The $\Delta^2$ is a killer....  Maybe we have to go back to using Bernstein's Inequality?}
  ...
  % Furthermore, for each $w\in C_p$ and each $y\in N^+_H(p)$, $y$ contributes at most $\deg^+_G(y)< 4d$ to $\sum_{\alpha} N_\alpha(w)$. By the definitions of $\Phi_w$ and $M_w$, $\sum_{\alpha\in \Phi_i} N_\alpha(w)\ge kM_w$.  Therefore, $w$ has at least $kM_w/(4d)$ neighbours $y$ in $N^+_H(p)$ such that $w\in N^+_G(y)$.  Therefore $w$ contributes at least $kM_i$ to $\sum_{y\in N^+_H(p)}\deg^+_G(y)= O(d^2\Delta)$.  Therefore $\sum_{w\in C_{p}} M_w = O(d^3\Delta/k)$.

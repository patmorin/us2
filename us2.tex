...
\begin{lem}\label{advanced_cairns}
  ...
  $d$-degenerate graph $G$ of maximum-degree $\Delta$ there exists a mapping $\rho:\mathcal{P}_\ell(G)\to V(G)$ such that
  \begin{compactenum}[(i)]
...
\begin{proof}
  Let $<$ be a $d$-degenerate ordering of $V(G)$, so that each vertex has at most $d$ neighbours that precede it in the order.  Consider some path $\Pi:=x_0,\ldots,x_r$ in $G$ of length $r\le\ell$.  Say that an edge $x_ix_{i+1}$ of $\Pi$ is a \defin{downstream} edge of $\Pi$ if $x_i < x_{i+1}$, otherwise it is an \defin{upstream} edge.  If at least half the edges of $\Pi$ are downstream edges of $\Pi_{vw}$ then set $\rho(\Pi):=x_0$, otherwise set $\rho(\Pi):=x_r$.
...
% \subsection{Degenerate Graphs of Small Maximum Degree}
%
% The same proof used to establish \cref{degenerate_and_degree} establishes the following generalization, which is useful for graphs of bounded degeneracy and moderate maximum degree:
%
...
\begin{thm}\label{l_degenerate_and_degree}
  For any integers $d\ge 2$ and $\ell\ge 2$ there exists a constant $c:=c(d,\ell)$ such that, for all integers $\Delta\ge d$ and $n\ge \Delta$, every $n$-vertex $d$-degenerate graph $G$ of maximum degree $\Delta$ has $\lrn(G)\le c\Delta^{?}\log n$.
\end{thm}

\begin{proof}[Proof Sketch]
  Let $G$ be an $n$-vertex $d$-degenerate graph of maximum degree $\Delta$ and let $k:=\Delta^{?}$. Proceed exactly as in the proof of \cref{degenerate_and_degree} by partitioning $V(G)$ into the sets $L_0,\ldots,L_q$ and defining the total order $<$ as before.  In the first phase, the vertices in $L_i$ will receive colours from a palette $\Phi_i$ of size $2k$, so the first phase will use at most $k(q+1)=O(k\log n)$ colours.  As before, the colours in $\Phi_{i}$ are all larger than the colours in $\Phi_{i-1}$, for each $i\in\{1,\ldots,q\}$.

  Let $\mathcal{P}$ contain all the undirected paths $x_0,\ldots,x_r$ in $\mathcal{P}_{\ell}(G)$ such that $x_0$ and $x_r$ are in the same layer $L_j$ and $\{x_1,\ldots,x_{r-1}\}\subseteq \bigcup_{i=0}^j L_i$.  Let $\rho:\mathcal{P}\to V(G)$ be the mapping given by \cref{advanced_cairns}, restricted to the paths in $\mathcal{P}$.  Let $\Pi:=x_0,\ldots,x_r$ be a path in $\mathcal{P}$.  We say that $\Pi$ is \defin{problematic} if $x_0$ and $x_r$ are assigned the same colour in the first phase of colouring.

  Although we have not yet completed the description of the first phase colouring, we know enough to prove the following claim:  Any $\ell$-violation $\Pi$ of the first stage colouring is a problematic path in $\mathcal{P}$.  Indeed, if $\Pi=x_0,\ldots,x_r$ is an $\ell$-violation, then the colour of $x_0$ and $x_r$ is the same, so $x_0$ and $x_r$ must be contained in $L_j$ for some $j\in\{0,\ldots,q\}$.  Furthermore the colour of $x_0$ and $x_r$ is at least as large as any colour used on $x_1,\ldots,x_{r-1}$, so no colour in $\Phi_{j+1},\ldots,\Phi_q$ appears at any vertex in $x_1,\ldots,x_{r-1}$.  Therefore, $\{x_1,\ldots,x_{r-1}\}\subseteq \bigcup_{i=0}^j L_i$.  Therefore $\Pi\in\mathcal{P}$ and $\Pi$ is problematic because $x_0$ and $x_r$ receive the same colour.

  We now describe the first phase colouring algorithm.  Consider the multigraph $G^*$ that, for each $vw\in V(G)$ contains as many copies of the edge $vw$ as there are undirected paths in $\mathcal{P}$ with endpoints $v$ and $w$.  The existence of $\rho$ implies that $G^*$ has an orientation in which each edge has out-degree $O(\Delta^{\lfloor\ell /2})$.  By \cref{orientation_to_degeneracy}, $G^*$ is $O(\Delta^{\lfloor\ell /2})$-degenerate.  Order the vertices of $G$ in the order $v_1,\ldots,v_n$ so that $v_i$ has degree $O(\Delta^{\lfloor\ell /2})$ in $G^*[\{v_1,\ldots,v_{i}]$.

  Let $\tau:\mathcal{P}\to V(G)$ be the mapping that maps $\Pi\in\mathcal{P}$ to $v_j$ if and only if the endpoints of $\Pi$ are $v_i$ and $v_j$ and $i < j$. Then $|\tau^{-1}(w)|=O(\Delta^{\lfloor\ell /2\rfloor})$  Consider some vertex $w\in L_j$.  For each $\alpha \in \Phi_j$, let $N_{\alpha}(w)$ be the number of paths in $\tau^{-1}(w)$ whose other endpoint (not $w$) is assigned the colour $\alpha$. If $w=v_j$ then $N_{\alpha}(w)$ is completely determined by the colours of $v_1,\ldots,v_{j-1}$.  If $w$ is assigned the colour $\alpha$ then exactly $N_{\alpha}(w)$ paths in $\tau^{-1}(w)$ paths will become problematic.  Therefore, $\sum_{\alpha\in\Phi_i} N_\alpha(w)=|\tau^{-1}(w)|=O(\Delta^{\lfloor\ell /2\rfloor})$.  As before, we choose $\Phi_w\subseteq\Phi_i$ of size $k$ so that $N_\alpha(w)=O(\Delta^{\lfloor\ell /2\rfloor}/k)$ for all $\alpha\in\Phi_w$. Let $M:=c\Delta^{\lfloor\ell /2\rfloor}/k$ where $c$ is large enough that $N_\alpha(w)\le M$ for all $w\in V(G)$ and all $\alpha\in \Phi_w$.   In the first phase, we choose the colour of $w$ uniformly from $\Phi_w$.  This completes the description of the first phase colouring algorithm.

  Consider a problematic path $\Pi:=x_0,\ldots,x_r$ and suppose, without loss of generality that $\tau(\Pi)=x_0$.  Then $x_r$ was coloured before $x_0$ and $\Pi$ became problematic precisely when $x_0$ was coloured.  When this happens we say that $x_0$ \defin{completes} $\Pi$ and we call $x_1$ a \defin{problematic} vertex.  Let $P$ be the set of all problematic vertices.
  Observe that, for any $\Pi\in\mathcal{P}$, the probability that $\tau(\Pi)$ completes $\Pi$ is at most $1/k$ since the colour of $\tau(\Pi)$ is chosen from a set of size $k$ and at most one element of this set is the colour of the other endpoint of $\Pi$.  Thus, for any $\Pi\in\mathcal{P}$ the probability that $\Pi$ is problematic is at most $1/k$.

  Let $H$ be the directed multigraph with vertex set $V(G)$ that, for each vertex $p$ of $G$ and each path $p\rightsquigarrow y$ in $\tau^{-1}(p)$ contains a copy of the edge $\overrightarrow{py}$.  Then the maximum out-degree of any vertex in $H$ is $O(\Delta^{\lfloor\ell /2\rfloor})$.  We say that an edge of $H$ is \defin{problematic} if the corresponding undirected path $\Pi$ is problematic.  Since each edge of $H$ is problematic with probability at most $1/k$, the expected number of problematic edges leaving any vertex $p$ is at most $O(\Delta^{\lfloor\ell /2\rfloor}/k)$.

  If the events $\{\text{``$e$ is problematic''}:e\in\operatorname{out-edges}_H(p)\}$ were mutually independent, then we could apply a Chernoff bound and be done.  Unfortunately, this is not the case since the choice of colour at a single vertex $w$ can trigger up to $M$ of these events.  Despite this, we will still be able to show that, with high probability, the maximum out-degree of any vertex in $H[P]$ is $O(\Delta^{\lfloor\ell /2\rfloor}\log n/k)$.  Some of the work in doing this involves applying a (somewhat specialist) concentration bound of a type developed by Bernstein for sums of independent random variables.  In the remainder of this proof we set up appropriate random variables.  The statement of the concentration bound and the calculations needed to apply it are in the appendix.

  \todo[inline]{Do we need randomization here at all?  Starting a new branch to try without it.}

  Let $p$ be an arbitrary vertex of $G$.  We want to study the random variable $X_p$ that counts the number of problematic paths in $\tau^{-1}(p)$.

  Suppose that some $y\in N^+_H(p)$ is problematic.  Then there exists some undirected path $\Pi:=x_0,\ldots,x_r$ in $\mathcal{P}$ that is problematic, $x_1=y$, and $\tau(\Pi)=x_0$.  If $x_0\in L_j$ then by the definition of $\mathcal{P}$, $y\in\bigcup_{i=0}^j L_i$.  Therefore, $x_0$ is one of the at most $4d$ vertices in $N_G(y)\cap L_j$.

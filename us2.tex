...
\begin{proof}[Proof of \cref{degenerate_and_degree}]
  ...
  \todo[inline]{There may actually be a problem here.  The value $M_w$ depends on $N^-_G(w)$.  It doesn't (and can't) take into account $|N^-_G(w)\cap P|$.  We need an upper bound on $\sum_{w\in C_p}M_w$.  Right now, the only upper bound I can see is $\sum_{w\in C_p} 4d|N^-_G(w)|/k \le |C_p|\cdot 4d\Delta= O(d^3\Delta^2)$.  The $\Delta^2$ is a killer....}

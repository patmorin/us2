...
\begin{proof}[Proof of \cref{degenerate_and_degree}]
  ...
  Therefore, $|\outn(v)|$ is dominated by $X:=\sum_{w> v} N'_{v,w}$ of independent random variables.  In the appendix, we show how to use one of Bernstein's Inequalities to prove that $\Pr\left(X\ge Cd^?\sqrt{\Delta}\log^{3/2} n\right)\le n^{\Omega(C^2)}} = o(1/n)$ for sufficiently large $C$.  This completes the proof.

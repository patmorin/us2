...
\begin{proof}[Proof of \cref{degenerate_and_degree}]
  ...
  First, observe that the maximum out-degree of any vertex in $\overrightarrow{G}$ is less than $4d$. Indeed, a vertex $v$ is contained in $L_i$ precisely because $\deg_{G[S_i]}(v)<4d$.  The out-degree bound then follows from the fact that $N_{\overightarrow{G}}(v)\subseteq S_i$, $\deg_{\overrightarrow{G}}(v)\le\deg_{G[S_i]}(v)<4d$.  We claim that for each vertex $v$, $\deg_{\overightarrow{H}}(v)\le (4d-1)(2\Delta+1)$.  To see this, suppose $v\in L_i$ and consider some edge $\overrightarrow{vw}$ of $\overrightarrow{H}$.  The edge  $\overrightarrow{vw}$ is of one of the following types:
  \begin{compactenum}[(T1)]
    \item $\overightarrow{vw}\in E(\overightarrow{G})$. Edges of this type contribute at most $\deg_{\overightarrow{G}}(v)\le 4d-1$ to $\deg_{\overightarrow{H}}(v)$.
